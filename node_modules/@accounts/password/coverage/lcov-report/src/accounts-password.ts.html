...
<html lang="en">
<head>
    <title>Code coverage report for src/accounts-password.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">All files</a> / <a href="index.html">src</a> accounts-password.ts
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">92.5% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>222/240</span>
      </div>
      <div class='fl pad1y space-right2'>
        <span class="strong">81.6% </span>
        <span class="quiet">Branches</span>
        <span class='fraction'>133/163</span>
      </div>
      <div class='fl pad1y space-right2'>
      ...
      <div class='fl pad1y space-right2'>
        ...
        <span class='fraction'>187/203</span>
      </div>
    </div>
    <p class="quiet">
...
<a name='L637'></a><a href='#L637'>637</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">2x</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">2x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { trim, isEmpty, pick, isString, isPlainObject, find, includes, defer } from 'lodash';
import {
  User,
  LoginUserIdentity,
  EmailRecord,
...
&nbsp;
  constructor(<span class="missing-if-branch" title="if path not taken" >I</span>options: AccountsPasswordOptions = {}) {
...
&nbsp;
  public async authenticate(params: LoginUserPasswordService): Promise&lt;User&gt; {
    ...
    if ((!isString(user) &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >!isPlainObject(user))</span> || !isString(password)) {
...
&nbsp;
    ...
    if (<span class="missing-if-branch" title="else path not taken" >E</span>getUserTwoFactorService(foundUser)) {
<span class="cstat-no" title="statement not covered" >      await this.twoFactor.authenticate(foundUser, code!);</span>
    }
&nbsp;
    return foundUser;
  }
...
&nbsp;
   ...
   */
  public addEmail(userId: string, newEmail: string, <span class="missing-if-branch" title="if path not taken" >I</span>verified = false): Promise&lt;void&gt; {
...
&nbsp;
    const emails = user.emails || <span class="branch-1 cbranch-no" title="branch not covered" >[];</span>
    if (
      !includes(
...
&nbsp;
    ...
    if (<span class="missing-if-branch" title="if path not taken" >I</span>this.options.invalidateAllSessionsAfterPasswordReset) {
      await this.db.invalidateAllSessions(user.id);
    }
&nbsp;
    if (<span class="missing-if-branch" title="if path not taken" >I</span>this.options.notifyUserAfterPasswordChanged) {
      const address = user.emails &amp;&amp; user.emails[0].address;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (!address) {
<span class="cstat-no" title="statement not covered" >        throw new Error(this.options.errors.noEmailSet);</span>
      }
&nbsp;
...
&nbsp;
    ...
    if (!user) {
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (this.server.options.ambiguousErrorMessages) {
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (user.username &amp;&amp; !this.options.validateUsername(user.username)) {
<span class="cstat-no" title="statement not covered" >      throw new Error(this.options.errors.invalidUsername);</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (user.email &amp;&amp; !this.options.validateEmail(user.email)) {
<span class="cstat-no" title="statement not covered" >      throw new Error(this.options.errors.invalidEmail);</span>
    }
&nbsp;
...
&nbsp;
    if (<span class="missing-if-branch" title="if path not taken" >I</span>user.email &amp;&amp; (await this.db.findUserByEmail(user.email))) {
      throw new Error(this.options.errors.emailAlreadyExists);
    }
&nbsp;
    if (<span class="missing-if-branch" title="if path not taken" >I</span>user.password) {
      <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.options.validatePassword(user.password)) {
<span class="cstat-no" title="statement not covered" >        throw new Error(this.options.errors.invalidPassword);</span>
      }
...
&nbsp;
      defer(async () =&gt; {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (this.options.sendVerificationEmailAfterSignup &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >user.email)</span>
<span class="cstat-no" title="statement not covered" >          this.sendVerificationEmail(user.email);</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      await this.server.getHooks().emit(ServerHooks.CreateUserError, user);</span>
...
&nbsp;
  private async passwordAuthenticator(
    ...
    const { username, email, id } = isString(user)
      ...
      : <span class="branch-1 cbranch-no" title="branch not covered" >this.toUsernameAndEmail({ ...user });</span>
&nbsp;
    let foundUser: User | null = null;
&nbsp;
    if (<span class="missing-if-branch" title="else path not taken" >E</span>id) {
      // this._validateLoginWithField('id', user);
<span class="cstat-no" title="statement not covered" >      foundUser = <span class="cstat-no" title="statement not covered" >await this.db.findUserById(id);</span></span>
    } else if (<span class="missing-if-branch" title="else path not taken" >E</span>username) {
      // this._validateLoginWithField('username', user);
<span class="cstat-no" title="statement not covered" >      foundUser = <span class="cstat-no" title="statement not covered" >await this.db.findUserByUsername(username);</span></span>
    } else if (<span class="missing-if-branch" title="if path not taken" >I</span>email) {
    ...
    if (!foundUser) {
      throw new Error(
        this.server.options.ambiguousErrorMessages
          ? <span class="branch-0 cbranch-no" title="branch not covered" >this.options.errors.invalidCredentials</span>
...
&nbsp;
    ...
    const pass: any = hashAlgorithm ? <span class="branch-0 cbranch-no" title="branch not covered" >hashPassword(password, hashAlgorithm) </span>: password;
    const isPasswordValid = await verifyPassword(pass, hash);
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!isPasswordValid) {
      throw new Error(
        this.server.options.ambiguousErrorMessages
          ? <span class="branch-0 cbranch-no" title="branch not covered" >this.options.errors.invalidCredentials</span>
...
&nbsp;
  private async hashAndBcryptPassword(password: string): Promise&lt;string&gt; {
    const hashAlgorithm = this.options.passwordHashAlgorithm;
    const hashedPassword = hashAlgorithm ? <span class="branch-0 cbranch-no" title="branch not covered" >hashPassword(password, hashAlgorithm) </span>: password;
...
&nbsp;
  ...
  private toUsernameAndEmail({ user, username, email, id }: any): any {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (user &amp;&amp; !username &amp;&amp; !email) {
      <span class="missing-if-branch" title="else path not taken" >E</span>if (isEmail(user)) {
        email = user;
        username = null;
      } else {
<span class="cstat-no" title="statement not covered" >        username = user;</span>
<span class="cstat-no" title="statement not covered" >        email = null;</span>
      }
    }
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="https://istanbul.js.org/" target="_blank">istanbul</a> at Fri Feb 14 2020 16:06:25 GMT+0100 (Central European Standard Time)
</div>
</div>
